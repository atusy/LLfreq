...
#' @param uncertainty2 logical; if \code{TRUE} (default), drawing 90 percent
...
lalgp_graph <- function(data,
  ...
  plot_data <- data.frame(age_X = p$age_X,
                          mean = p$p_mean,
                          p_0025 = p$p_0025,
                          p_0975 = p$p_0975,
                          p2_0025 = p$p2_0025,
  ...
  if(is.na(y_max) == T){
    ...
    y_max_ceiling <- ceiling(1000*y_max) / 1000
  } else {
    y_max_ceiling <- y_max
  }
  if(y_max < 0) {
  ...
  if(uncertainty2 == T){
    polygon(
      x = c(plot_data$age_X , rev(plot_data$age_X ), plot_data$age_X[1]),
      y = c(plot_data$p2_0025, rev(plot_data$p2_0975), NA),
      col = rgb(0, 0, 0,alpha = .3),
      lty = "blank"
    )
  }
